...
const int maxNum = 5; // максимальная оценка
...
void addStudent(vector<Student>& students) {
    ...
    for (int i = 0; i < numGrades; ++i) {
        ...
        int grade;
        while (!(cin >> grade) || grade < 1 || grade > maxNum) { // проверка ввода оценки
            cout << "Недопустимый ввод. Попробуйте еще раз: ";
            cin.clear();
            cin.ignore(numeric_limits<streamsize>::max(), '\n');
        }
...
void addGrades(vector<Student>& students) { // функция для обавления оценок ученику
    ...
    for (int i = 0; i < numGrades; ++i) { // добавляем каждую оценку
        cout << "Введите оценку " << i + 1 << ": ";
        int grade;
        while (!(cin >> grade) || grade < 1 || grade > maxNum) { // проверка корректность оценки
            cout << "Недопустимый ввод. Попробуйте еще раз: ";
            cin.clear();
            cin.ignore(numeric_limits<streamsize>::max(), '\n');
        }
...
void modifyGrade(vector<Student>& students) { // функция для изменения оценки
    ...
    int newGrade;
    while (!(cin >> newGrade) || newGrade < 1 || newGrade > maxNum) {
        cout << "Недопустимый ввод. Попробуйте еще раз: ";
        cin.clear();
        cin.ignore(numeric_limits<streamsize>::max(), '\n');
    }
...
int main() {
    ...
    if (outFile.is_open()) {
        ...
        cout << "Ошибка: не удалось сохранить оценки студентов." << endl; // Уведомление об ошибке открытия файла
    }
    return 0;
}